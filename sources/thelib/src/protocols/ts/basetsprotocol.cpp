...
void BaseTsProtocol::FreePidDescriptor(PIDDescriptor *pPIDDescriptor) {
...
bool BaseTsProtocol::Initialize(Variant &parameters) {
	GetCustomParameters() = parameters;
	return true;
}

bool BaseTsProtocol::AllowFarProtocol(uint64_t type) {
	return true;
}

bool BaseTsProtocol::AllowNearProtocol(uint64_t type) {
	FATAL("This protocol doesn't allow any near protocols");
	return false;
}

void BaseTsProtocol::SetApplication(BaseClientApplication *pApplication) {
...
BaseTSAppProtocolHandler *BaseTsProtocol::GetProtocolHandler() {
	return _pProtocolHandler;
}

bool BaseTsProtocol::SignalInputData(int32_t recvAmount) {
	ASSERT("OPERATION NOT SUPPORTED");
	return false;
}

bool BaseTsProtocol::SignalInputData(IOBuffer &buffer) {
	if (_chunkSize == 0) {
...
bool BaseTsProtocol::DetermineChunkSize(IOBuffer &buffer) {
...
bool BaseTsProtocol::ProcessPacket(uint32_t packetHeader,
...
bool BaseTsProtocol::ProcessPidTypePAT(uint32_t packetHeader,
...
bool BaseTsProtocol::ProcessPidTypePMT(uint32_t packetHeader,
		PIDDescriptor &pidDescriptor, uint8_t *pBuffer, uint32_t &cursor,
		uint32_t maxCursor) {
	//1. Advance the pointer field
	if (TS_TRANSPORT_PACKET_IS_PAYLOAD_START(packetHeader)) {
...
uint32_t BaseTsProtocol::GetChunkSize() {
	return _chunkSize;
}

void BaseTsProtocol::SetStepByStep(bool stepByStep) {
