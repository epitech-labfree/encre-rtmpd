...
TSPacket::TSPacket(BaseProtocol* protocol, uint16_t pid, double timestamp) {
	...
	// audio->elementaryPID = 69;
	// audio->esInfoLength = 0;
	// ADD_VECTOR_END(_programStreamType, audio);
...
bool TSPacket::CreatePacket(uint8_t* pData, uint32_t dataLength, bool isAudio, bool pat, bool pmt) {
	...
	header |= (_transportPriority & 0x1) * 0x200000;
...
bool TSPacket::sendData() {
	if (_pProtocol != NULL && TAG_KIND_OF(_pProtocol->GetType(), PT_INBOUND_TS)) {
		return ((InboundTSProtocol*)_pProtocol)->SendRawData(GETIBPOINTER(_packet), GETAVAILABLEBYTESCOUNT(_packet));
	} else {
		return false;
	}
}
